#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Plot MatMul benchmark results: runtime, speedup, efficiency.
...
from statistics import median, pstdev, mean, stdev
import matplotlib
matplotlib.use("Agg")
...
def plot_runtime(byN, Ns, Ps, outdir, tag, variant):
    ...
    plt.title(f"MatMul ({label}): Runtime vs N", fontsize=12)
...
def plot_speedup(byN, Ns, Ps, outdir, tag, variant):
    ...
    label = VARIANT_LABELS.get(variant, variant)
    plt.title(f"MatMul ({label}): Speedup vs N (baseline P={baseP})", fontsize=12)
    plt.xlabel("Matrix dimension N", fontsize=11)
...
def plot_efficiency(byN, Ns, Ps, outdir, tag, variant):
    ...
    label = VARIANT_LABELS.get(variant, variant)
    plt.title(f"MatMul ({label}): Efficiency vs N (baseline P={baseP})", fontsize=12)
    plt.xlabel("Matrix dimension N", fontsize=11)
...
def plot_compare_speedup(data, Ns, outdir, tag):
    ...
    for variant in ("super", "ghc", "parpseq"):
        ...
        si += 1
    plt.title("Matrix Multiply: Best Speedup vs N", fontsize=12)
    plt.xlabel("Matrix dimension N", fontsize=11)
