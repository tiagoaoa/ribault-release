# What If Haskell's Parallelism Problem Isn't the Language — It's the Runtime?
...
Tags are computed as integers: `tag = parent * R + k`, where R is the tag radix (default 9) and k identifies the call site. This supports approximately `floor(log_9(2^63))` ≈ 19 levels of recursive nesting — more than enough for the coarse-grained decompositions Ribault targets.
...
At P=16, TALM reaches **8.13x** speedup over the sequential baseline. Both GHC variants plateau below **1.21x** — they barely improve over sequential despite having 16 cores available. The bottleneck: concurrent heap allocation across 14 tasks triggers stop-the-world GC so frequently that at P≥4, GHC effectively serialises execution.
...
In TALM, you state the recurrence: block (i,j) depends on blocks (i-1,j) and (i,j-1). Trebuchet fires each block the instant both tokens arrive. The wavefront advances purely by data availability — no diagonal iteration loop, no explicit barriers.
...
The practical consequence: **supers must encapsulate O(ms) or more of sequential work** to amortise the dataflow dispatch overhead. Programs that require fine-grained interleaving of parallel coordination and computation — like lock-step stencil computations or the iterative Fibonacci base case — are not well suited to the current architecture.
...
The full release — compiler, runtime, 28 test programs, benchmark harness, and all paper data — is available as a self-contained package:
...
**Hardware dataflow accelerators.** The software Trebuchet runtime pays ~0.5 ms per dataflow operation. Targeting hardware accelerators — like the SambaNova SN40L's dataflow architecture — could eliminate the matching-store overhead entirely, recovering the performance lost on fine-grained benchmarks like Fibonacci.
...
*Ribault is developed at UFES/UERJ. The compiler is the undergraduate thesis of Ricardo Magalhaes Santos Filho, advised by Prof. Alberto Ferreira de Souza and co-advised by Prof. Tiago A.O. Alves. The TALM runtime was developed at COPPE/UFRJ.*
